-- Fix Exchange Form for Proper Bidirectional Exchange
-- Add partner's desired off day field

-- Add new field for partner's desired off day
...
-- Update the bidirectional swap function to use the new field structure
...
BEGIN
    ...
    -- 3. Requester works on partner's desired off date (if different weeks, need separate schedule)
    IF partner_off_week != requester_off_week THEN
        -- Different weeks - need to update requester's schedule for partner's week
        DECLARE
        ...
        END;
    ELSE
        -- Same week - requester already working on partner's day (just ensure it's working)
        EXECUTE format('UPDATE work_schedules SET %I = ''working'' WHERE id = $1', partner_off_day_column)
        USING requester_off_schedule_id;
    END IF;
    
    -- 4. Partner works on requester's desired off date (if different weeks, need separate schedule)  
    IF requester_off_week != partner_off_week THEN
        -- Different weeks - need to update partner's schedule for requester's week
        DECLARE
        ...
        END;
    ELSE
        -- Same week - partner already working on requester's day (just ensure it's working)
        EXECUTE format('UPDATE work_schedules SET %I = ''working'' WHERE id = $1', requester_off_day_column)
        USING partner_off_schedule_id;
    END IF;
...
-- Update admin approval function to use the new bidirectional structure
...
-- Show current exchange requests and highlight missing partner dates
SELECT 
    'Exchange Requests - Check for Missing Partner Dates:' as info,
    r.id,
    r.employee_name as requester,
    e.name as partner,
    r.exchange_from_date as "Requester From",
    r.exchange_to_date as "Requester Wants Off",
    r.partner_desired_off_date as "Partner Wants Off",
    CASE 
        WHEN r.partner_desired_off_date IS NULL THEN '❌ MISSING: Partner desired off date'
        ELSE '✅ Complete bidirectional exchange data'
    END as data_status,
    r.status,

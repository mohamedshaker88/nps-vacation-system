...
-- This script will ensure exchange partners receive notifications when requests are submitted

BEGIN;

-- Step 1: Disable RLS on notifications table to avoid permission issues
ALTER TABLE notifications DISABLE ROW LEVEL SECURITY;

-- Step 2: Drop any existing notification triggers and functions
...
DROP FUNCTION IF EXISTS create_notification_bypass_rls(bigint, bigint, varchar, varchar, text) CASCADE;

-- Step 3: Create a simple, reliable notification creation function
...
-- Step 4: Create a trigger function that ALWAYS works
...
-- Step 6: Update dataService function to work with manual calls
...
-- Step 7: Create notifications for any existing requests that don't have them
DO $$
DECLARE
    request_record RECORD;
...
-- Step 8: Update the get_pending_exchange_approvals function
...
-- Step 9: Test the system with a manual notification
DO $$
DECLARE
...
SELECT 'NOTIFICATION SYSTEM FIX COMPLETE!' as result; 
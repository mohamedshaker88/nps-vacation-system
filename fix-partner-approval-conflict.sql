-- Fix Partner Approval Function Conflict
...
-- Also drop related functions that might conflict
...
BEGIN
    ...
    IF request_row.id IS NULL THEN
        RETURN jsonb_build_object(
            ...
            'debug', jsonb_build_object('request_id', p_request_id)
        );
    END IF;
    ...
    IF request_row.exchange_partner_id != p_employee_id THEN
        RETURN jsonb_build_object(
            ...
            'debug', jsonb_build_object(
                ...
                'expected_partner_id', request_row.exchange_partner_id,
                'provided_employee_id', p_employee_id
            )
        );
    END IF;
    ...
    IF request_row.exchange_partner_approved IS NOT NULL THEN
        RETURN jsonb_build_object(
            ...
            'debug', jsonb_build_object(
                'current_response', request_row.exchange_partner_approved,
                'responded_at', request_row.exchange_partner_approved_at
            )
        );
    END IF;
    ...
    RETURN jsonb_build_object(
        ...
        'requester_name', requester_employee.name,
        'partner_name', partner_employee.name
    );
...
-- Recreate the get_pending_exchange_approvals function with consistent types
...
DECLARE
    test_result JSONB;
BEGIN
    -- Try to get a sample exchange request to test
    IF EXISTS (SELECT 1 FROM requests WHERE type = 'Exchange Off Days' LIMIT 1) THEN
        RAISE NOTICE 'approve_exchange_request function recreated successfully';
    ELSE
        RAISE NOTICE 'No exchange requests found to test, but function created successfully';
    END IF;
END $$;

-- Show current exchange requests status
SELECT 
    'Current Exchange Requests After Fix:' as info,
    r.id,
    ...
    r.exchange_to_date,
    CASE 
        WHEN r.exchange_partner_approved IS NULL THEN '⏳ Waiting for partner (' || e.name || ')'
